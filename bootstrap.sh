...
if which git > /dev/null 2>&1; then
    git clone https://github.com/theaudaxproject/electrumx-installer ~/.electrumx-installer
    cd ~/.electrumx-installer/
else
    which wget > /dev/null 2>&1 && which unzip > /dev/null 2>&1 || { echo "Please install git or wget and unzip" && exit 1 ; }
    wget https://github.com/theaudaxproject/electrumx-installer/archive/master.zip -O /tmp/electrumx-master.zip
